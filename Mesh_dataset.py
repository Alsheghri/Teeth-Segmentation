...
class Mesh_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Curvature calculations using Trimesh
        #mesh_curvatures_minLen = getCurvatureChannels(mesh0, sphereRadius='min')
        #mesh.addCellArray(mesh_curvatures_minLen, 'CurvatureMinLength')
        #mesh_curvatures_maxLen = getCurvatureChannels(mesh0, sphereRadius='max')
        #mesh.addCellArray(mesh_curvatures_maxLen, 'CurvatureMaxLength')
        ...
        #curvaturesMinLen = mesh.getCellArray('CurvatureMinLength').copy()  # need to copy, they use the same memory address
        #curvaturesMaxLen = mesh.getCellArray('CurvatureMaxLength').copy()  # need to copy, they use the same memory address
        ...
        #mesh1 = Easy_Mesh()
        #mesh1.cells = cells
        #mesh1.update_cell_ids_and_points()
        #mesh1.to_obj('tmp1.obj')
        #################################################
        ...
        #cMinLenMeans = curvaturesMinLen.mean(axis=0)
        #cMinLenStds = curvaturesMinLen.std(axis=0)
        #cMaxLenMeans = curvaturesMaxLen.mean(axis=0)
        #cMaxLenStds = curvaturesMaxLen.std(axis=0)
        ...
        for i in range(3):
            ...
            #curvaturesMinLen[:,i] = (curvaturesMinLen[:,i] - cMinLenMeans[i]) / cMinLenStds[i]
            #curvaturesMaxLen[:,i] = (curvaturesMaxLen[:,i] - cMaxLenMeans[i]) / cMaxLenStds[i]
...
if __name__ == '__main__':
    ...
    #next_item = next(selfsupDataLoader)
